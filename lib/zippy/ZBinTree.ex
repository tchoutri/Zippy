defmodule Zippy.ZBinTree do
  ...
  def   replace({thread, nil}, value),                             do: {thread, {:fork, value, nil, nil}}
  def   replace({thread, {:fork, _oldvalue, left, right}}, value), do: {thread, {:fork, value, left, right}}
  ...
  def   add_left(zipper, new_branch) do
    zipper
    |> left
    |> replace(new_branch)
    |> up
